...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	if (pCommObject)
	{
		...
		pCommObject->lSleepTimeout = 1000;
	...
	BOOL bSuccess = CreateProcess(TEXT("E:\\Test\\VS2013\\IPCDemo\\Debug\\IPCWorker.exe"), TEXT("Red"), NULL, NULL, FALSE, NULL, NULL, NULL, &startupInfoRed, &processInformationRed);
	if (!bSuccess)
	{
		cout << "Cannot create procee red!" << endl << "Error:\t" << GetLastError() << endl;
		UnregisterClass(WINDOW_CLASS_NAME, GetModuleHandle(NULL));
	...
	bSuccess = CreateProcess(TEXT("..\\Debug\\IPCWorker.exe"), TEXT("Blue"), NULL, NULL, FALSE, NULL, NULL, NULL, &startupInfoBlue, &processInformationBlue);
	if (!bSuccess)
	{
		cout << "Cannot create procee blue!" << endl << "Error:\t" << GetLastError() << endl;
		UnregisterClass(WINDOW_CLASS_NAME, GetModuleHandle(NULL));
		CloseHandle(hMapping);
		CloseHandle(hMutex);
		return 1;
...
HWND InitializeWnd()
{
	...
	HWND hButton = CreateWindow(TEXT("BUTTON"), TEXT("Close"), WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON | WS_TABSTOP, 275, 230, 100, 25, hWnd, (HMENU)BUTTON_CLOSE, wndEx.hInstance, NULL);
